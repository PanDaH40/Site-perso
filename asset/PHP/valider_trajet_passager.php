...
// Récupérer et valider les paramètres essentiels
...
// Vérifier que l'ID réservation est valide et que l'action est 'valider' ou 'probleme'
...
try {
    ...
    // Vérifier que la réservation existe et appartient bien à l'utilisateur connecté
    ...
    if ($action === 'valider') {
        // Mise à jour de la réservation : validation passager, note, avis, date validation
        ...
        // Récupérer conducteur et passager pour insérer ou mettre à jour l'avis
        $stmtCond = $pdo->prepare(
            "SELECT t.conducteur_id, r.passager_id 
             FROM trajets t 
        ...
        if ($dataCond && $dataCond['conducteur_id'] && $dataCond['passager_id']) {
            // Insérer ou mettre à jour l'avis du passager sur le conducteur
    ...
    } else { // action = 'probleme' : signaler un problème
        if (!$commentaire) {
            http_response_code(400);
        ...
        try {
            // Marquer la validation passager comme -1 (problème signalé)
            $pdo->prepare("UPDATE reservations SET validation_passager = -1, date_validation = NOW() WHERE id = ?")
                ->execute([$reservationId]);

            // Récupérer l'ID du conducteur lié à la réservation
            ...
            // Insérer un signalement dans la table signalements
            $pdo->prepare(
        ...
        // Notification par mail aux administrateurs
    ...
    // Après validation, vérifier si tous les passagers ont validé pour créditer le conducteur
    if ($action === 'valider') {
        $stmt = $pdo->prepare(
    ...
    // En cas d'erreur serveur, log et message générique
    http_response_code(500);
