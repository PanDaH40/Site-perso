...
// Vérification sécurité : id utilisateur passé en GET
...
try {
    ...
    // Calcul roleConducteur (présence des infos véhicule)
    $row['roleConducteur'] = !empty($row['marque_vehicule']) || !empty($row['modele_vehicule']) || !empty($row['carburant']);
    // Tu peux adapter rolePassager selon ta logique
    $row['rolePassager'] = false;

    // Récupérer les 5 derniers avis reçus avec auteur
    $reqAvis = $pdo->prepare("
    ...
    // Infos session + avis personnel (si connecté et autre profil)
