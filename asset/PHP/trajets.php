...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$userId) { echo json_encode(['error' => 'Utilisateur non connecté']); exit; }
...
if (isset($_GET['all']) && $_GET['all'] === '1') {
    try {
        ...
        if (!empty($_GET['depart']))  { $where[] = "t.depart LIKE ?";  $params[] = '%'.$_GET['depart'].'%'; }
        if (!empty($_GET['arrivee'])) { $where[] = "t.arrivee LIKE ?"; $params[] = '%'.$_GET['arrivee'].'%'; }
        if (!empty($_GET['date']))    { $where[] = "t.date = ?";       $params[] = $_GET['date']; }
        ...
        if (!empty($_GET['jetons_max'])) { $where[] = "t.jetons <= ?"; $params[] = floatval($_GET['jetons_max']); }
...
";
        ...
        // Date alternative si aucun résultat
        if (empty($all) && !empty($_GET['date']) && !empty($_GET['depart']) && !empty($_GET['arrivee'])) {
            $altWhere  = [];
            $altParams = [];
            $altWhere[] = "t.depart LIKE ?";  $altParams[] = '%'.$_GET['depart'].'%';
            $altWhere[] = "t.arrivee LIKE ?"; $altParams[] = '%'.$_GET['arrivee'].'%';
            $altWhere[] = "t.date > ?";       $altParams[] = $_GET['date'];
            ...
            if (!empty($_GET['places_min'])) { $altWhere[]="(t.places - (SELECT COALESCE(SUM(r.places_reservees),0) FROM reservations r WHERE r.trajet_id = t.id AND r.statut = 'valide')) >= ?"; $altParams[]=(int)$_GET['places_min']; }
            if (!empty($_GET['jetons_max'])) { $altWhere[]="t.jetons <= ?"; $altParams[]=(float)$_GET['jetons_max']; }
            if (!empty($_GET['note_min']))   { $altWhere[]="((SELECT AVG(a.note) FROM avis a WHERE a.utilisateur_id = t.conducteur_id)) >= ?"; $altParams[]=(float)$_GET['note_min']; }

            $sqlAlt = "SELECT t.date FROM trajets t ".(count($altWhere) ? "WHERE ".implode(" AND ", $altWhere):"")." ORDER BY t.date ASC, t.heure ASC LIMIT 1";
...
try {
    ...
    foreach ($proposes as &$tr) {
        $qr = $pdo->prepare("SELECT i.prenom, i.nom, r.places_reservees
             FROM reservations r
             JOIN inscrits i ON i.id = r.passager_id
             WHERE r.trajet_id = ? AND r.statut = 'valide'");
        $qr->execute([(int)$tr['id']]);
    ...
    foreach ($reserves as &$t) { $t['statut_passager'] = 'Réservé '.$t['places_reservees'].' place(s)'; }

    // c) Demandes en attente sur mes trajets (conducteur)
    $stmt3 = $pdo->prepare("
