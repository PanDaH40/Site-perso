document.addEventListener('DOMContentLoaded', () => {
  // Récupère l'id du conducteur depuis l'URL (ex: ?id=123)
  const params = new URLSearchParams(window.location.search);
  const conducteurId = params.get('id');

  // Sélecteurs DOM des éléments où afficher les infos et l'historique
  ...
  // Vérification que tous les éléments existent dans le DOM
  if (!titreEl || !infoConducteurEl || !listeEl || !aucunEl) {
    alert("Erreur dans la structure HTML de la page !");
    return; // Stoppe l’exécution si éléments manquants
  }

  // Vérification que l'ID conducteur est présent dans l'URL
  if (!conducteurId) {
    titreEl.textContent = "Conducteur inconnu";
    aucunEl.textContent = "Identifiant du conducteur manquant.";
    aucunEl.classList.remove('d-none'); // Affiche le message d’erreur
    return;
  }
  ...
  fetch(`/PHP/get_public_profile.php?id=${encodeURIComponent(conducteurId)}`)
    ...
    .then(data => {
      if (data.error) {
        // Affiche une alerte en cas d’erreur serveur
        infoConducteurEl.innerHTML = `<div class="alert alert-danger">${data.error}</div>`;
        return;
      }
      // Affiche la photo et le nom du conducteur
      ...
      `;
      // Met à jour le titre de la page avec le nom du conducteur
  ...
  fetch(`/PHP/historique_conducteur.php?id=${encodeURIComponent(conducteurId)}`)
    ...
    .then(data => {
      ...
      if (!data.trajets || !Array.isArray(data.trajets) || data.trajets.length === 0) {
        // Aucun trajet trouvé : affiche message
        listeEl.innerHTML = "";
        aucunEl.textContent = "Aucun trajet terminé trouvé pour ce conducteur.";
        aucunEl.classList.remove('d-none');
        return;
      ...
      // Il y a des trajets : masque le message d’absence
      aucunEl.classList.add('d-none');
      aucunEl.textContent = "";

      // Pour chaque trajet, crée une carte d’affichage
...
//       // S'il y a des trajets, on masque le message d'absence
//       aucunEl.classList.add('d-none');
//       aucunEl.textContent = "";
