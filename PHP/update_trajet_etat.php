...
$trajetId   = isset($input['trajet_id']) ? (int)$input['trajet_id'] : 0;
$nouvelEtat = $input['etat'] ?? '';

if (!$trajetId || !$nouvelEtat) {
...
try {
    ...
    if (!in_array($nouvelEtat, $transitions[$etatActuel], true)) {
        http_response_code(400);
    ...
    /*
    |--------------------------------------------------------------------------
    | 🔥 NOUVEAU : Quand un trajet passe en "termine",
    | on change les réservations en "terminee".
    | → Elles disparaissent alors automatiquement du dashboard.
    |--------------------------------------------------------------------------
    */
    if ($nouvelEtat === 'termine') {
        $stmt = $pdo->prepare("
            UPDATE reservations
            SET statut = 'terminee'
            WHERE trajet_id = ? AND statut = 'valide'
        ");
        $stmt->execute([$trajetId]);
    }
    ...
    error_log("Erreur update trajet: " . $e->getMessage());
