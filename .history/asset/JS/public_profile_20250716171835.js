document.addEventListener('DOMContentLoaded', () => {
  ...
  // Bouton "Donner un avis" et formulaire avis
  ...
  // Bouton "Envoyer un message" et modal (sélecteurs existants dans HTML)
  ...
  // Initialise modal Bootstrap
  const sendMessageModal = new bootstrap.Modal(sendMessageModalEl);
  ...
  // Création formulaire avis (comme avant, inchangé)
  ...
  fetch(`asset/PHP/get_public_profile.php?id=${encodeURIComponent(userId)}`)
    ...
    .then(data => {
      ...
      // Avis + bouton donner avis + formulaire (caché)
      avisSection.innerHTML = '';
      avisSection.appendChild(btnDonnerAvis);
      avisSection.appendChild(formulaireAvisContainer);

      // Affiche derniers avis
      ...
      // Affiche bouton "Donner un avis" si connecté et profil différent
      ...
      // Gère l’ouverture du modal "Envoyer un message"
      if (btnEnvoyerMessage && sendMessageModal) {
        btnEnvoyerMessage.onclick = () => {
          modalMessageText.value = ''; // reset textarea
      ...
      // Gestion de l’envoi du message via formulaire modal
      if (sendMessageForm) {
