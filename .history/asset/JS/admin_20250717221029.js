document.addEventListener("DOMContentLoaded", () => {
      ...
      .then(data => {
        ...
        data.users.forEach(u => {
          ...
          tr.innerHTML = `
            ...
            <td>
              ${u.admin == 1 ? '<span class="badge bg-dark">Admin</span>' : ''}
            </td>
            <td>
  ...
  window.adminEditJetons = function(id, currentJetons) {
  const nb = prompt("Combien de jetons à ajouter/enlever ? (ex : -10 pour retirer 10)", "10");
  if (!nb || isNaN(nb)) return;
  const action = nb > 0 ? 'add' : 'remove';
  fetch('asset/PHP/admin_jetons.php', {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
    body: JSON.stringify({id, nb_jetons: Math.abs(nb), action})
  })
  .then(r=>r.json()).then(data=>{
    alert(data.success ? "Opération réussie" : data.error);
    // recharge la liste si besoin
  });
};
