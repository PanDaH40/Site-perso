...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        (data.trajets_proposes || []).forEach(t => {
          const noms = (t.reservataires || []).map(r => `${r.prenom} ${r.nom} (${r.places_reservees})`).join(', ');
        ...
        (data.trajets_reserves || []).forEach(t => {
          // Calculer le nom du conducteur (champ 'conducteur' prioritaire)
          const conducteur = t.conducteur
            ? t.conducteur
            : [t.conducteur_prenom, t.conducteur_nom].filter(Boolean).join(' ');
          ...
          row.innerHTML = `
            ...
            <td>
              <button class="btn btn-sm btn-outline-warning" data-id="${t.id}" data-action="cancel">
                ❌ Annuler
              </button>
            </td>
          `;
  ...
  document.body.addEventListener('click', e => {
    ...
    if (action === 'edit' && editForm) {
      ...
      [editForm.editDate.value, editForm.editHeure.value] = cells[0].textContent.split(' ');
  ...
  if (editForm) {
    editForm.addEventListener('submit', e => {
      e.preventDefault();
      fetch('./asset/PHP/update_trajet.php', { method: 'POST', body: new FormData(editForm), credentials: 'same-origin' })
        .then(res => res.json())
