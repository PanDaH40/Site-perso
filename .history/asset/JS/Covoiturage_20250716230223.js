...
function rechercherTrajets() {
  ...
  const prixMax = document.getElementById("prixMax")?.value.trim() || "";
  const noteMin = document.getElementById("noteMin")?.value.trim() || "";
  ...
  fetch(url, { credentials: 'include' })
    .then(res => res.json())
    .then(data => {
      if ((data.all_trajets && data.all_trajets.length > 0) || (Array.isArray(data) && data.length > 0)) {
        afficherDateAlternative(''); // cache message date alternative
        afficherTrajets(data.all_trajets || data);
      } else {
        if (data.date_alternative) {
          afficherDateAlternative(data.date_alternative);
        } else {
          afficherDateAlternative('');
        }
        afficherTrajets([]);
      }
    })
