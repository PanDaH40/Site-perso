...
function confirmerReservation(trajetId) {
  // stocke l'ID dans le bouton du modal
  const btnConfirmer = document.getElementById('btnConfirmerReservation');
  btnConfirmer.setAttribute('data-trajet-id', trajetId);

  // (optionnel) afficher les infos dans le modal
  document.getElementById('modalBody').innerHTML = `<p>Trajet sélectionné #${trajetId}</p>`;
