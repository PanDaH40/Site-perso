document.addEventListener("DOMContentLoaded", function () {
  // --- Affichage du nom de l'utilisateur (existant) ---
  const welcomeMsg = document.getElementById("welcomeMsg");
  if (welcomeMsg) {
    fetch("./asset/PHP/user.php")
      .then(res => res.json())
      .then(data => {
        if (data.nom_complet) {
  ...
  // --- Section Page d'accueil dynamique ---
  const accueilDiv = document.getElementById("accueil");
  if (accueilDiv) {
    accueilDiv.innerHTML += `
      <p>Vous êtes connecté(e) sur votre espace personnel.</p>
      <p>Voici un résumé de vos activités récentes :</p>
    `;
  }
  ...
  if (messagesDiv) {
    messagesDiv.innerHTML = ""; // vide avant de remplir
