...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function chargerTrajets() {
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        // On affiche ici les trajets proposés (conducteur)
  ...
  // Afficher le formulaire d'ajout si utilisateur connecté (ton critère localStorage ici)
  ...
  // Gestion du formulaire d'ajout de trajet via fetch POST
  if (trajetForm) {
      ...
      fetch("./asset/PHP/trajets.php", {
        ...
        .then(data => {
          if (data.success) {
            ...
            trajetForm.reset();
            chargerTrajets(); // Recharge la liste des trajets après ajout
          } else {
  ...
  // Recherche avec filtrage côté client sur trajets déjà chargés
  if (searchForm) {
