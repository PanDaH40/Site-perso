...
async function chargerTrajetsAValider() {
  try {
    ...
    // Ajout des listeners sur les boutons valider
...
function ouvrirModalReservation(trajetId) {
  ...
  // --- CORRECTION : On nettoie les anciens handlers pour éviter le double-envoi ---
  const btnConfirmer = document.getElementById("btnConfirmerReservation");
  // Supprime tout ancien event listener
  ...
  btnConfirmerNew.onclick = function () {
    ...
    console.log('Places sélectionnées:', places); 
    console.log('Valeur envoyée au fetch :', places);
// debug
