    ...
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        (data.trajets_proposes || []).forEach(t => {
          let statutConducteur;
          if (t.total_reservations > 0) {
            // Construire la liste des réservataires
            const noms = Array.isArray(t.reservataires) && t.reservataires.length
              ...
              : '';
            statutConducteur = noms
              ? `${t.total_reservations} place(s) réservée(s): ${noms}`
              : `${t.total_reservations} place(s) réservée(s)`;
          } else {
            statutConducteur = 'Aucune réservation';
          }
