document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("./asset/PHP/check_session.php", { credentials: "same-origin" })
    ...
    .then(data => {
      if (data.connected) {
        ...
        if (userStatus) userStatus.textContent = "Vous n'êtes pas connecté. Veuillez vous connecter pour accéder au dashboard.";
      }
    })
    .catch(err => {
  ...
  function chargerTrajets() {
    ...
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .catch(err => {
        ...
        alert("Erreur lors du chargement des trajets : " + err.message);
      });
  }
  ...
  if (editForm) {
    editForm.addEventListener('submit', e => {
      ...
      fetch('./asset/PHP/update_trajet.php', { method: 'POST', body: formData })
        .then(res => res.json())
        .then(d => { if (d.success) { editModal.hide(); chargerTrajets(); } else alert('Erreur modification : ' + d.error); })
        .catch(err => { console.error('Erreur update trajet:', err); alert('Erreur serveur.'); });
    });
  }
});
