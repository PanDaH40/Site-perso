document.addEventListener("DOMContentLoaded", () => {
  ...
  // Charger les trajets proposés et réservés
    ...
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        if (proposes.length === 0) {
          ...
          proposes.forEach(t => {
            // Nom des réservataires
            const noms = Array.isArray(t.reservataires) && t.reservataires.length
              ? t.reservataires.map(r => `${r.prenom} ${r.nom}`).join(', ')
              : 'Aucun';
            const statut = t.total_reservations > 0
              ? `${t.total_reservations} place(s) réservée(s)<br><strong>Par :</strong> ${noms}`
              : "Aucune réservation";
        ...
        if (reserves.length === 0) {
          ...
          reserves.forEach(t => {
            const statut = t.statut_passager || t.statut;
            const row = document.createElement("tr");
            row.innerHTML = `
              <td>${t.date} ${t.heure}</td>
              <td>${t.depart}</td>
  ...
  // Événements éditer, supprimer, annuler
