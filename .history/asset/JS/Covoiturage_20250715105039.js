...
// Détection des réponses non-JSON pour rediriger vers la connexion
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Envoie POST pour création de trajet
  if (form) {
    form.addEventListener('submit', async e => {
      ...
      try {
        ...
        // Si response HTML, rediriger
        if (text.trim().startsWith('<')) {
          return redirectToLogin();
        }
        const result = JSON.parse(text);
        if (result.error) {
          if (result.error === 'Utilisateur non authentifié') return redirectToLogin();
  ...
  function afficher(trajets) {
      ...
      card.innerHTML = `
        <div class="card">
          <div class="card-body">
            ...
            <p>Date: ${t.date}<br>Heure: ${t.heure}<br>Places restantes: ${placesRestantes}<br><strong>Prix: ${t.prix.toFixed(2)} €</strong></p>
    ...
    document.querySelectorAll('.btn-reserver').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        `;
        btnConfirmer.dataset.trajetId = id;
  ...
  async function loadTrajets() {
    try {
      ...
      let url = `${API_BASE}/trajets.php` + (params.has('all') ? '?all=1' : '');
      ...
      if (data.error) {
        if (data.error === 'Utilisateur non authentifié') return redirectToLogin();
