...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (editForm) {
    editForm.addEventListener('submit', async e => {
      ...
      const id     = editForm.trajetId.value;
      const date   = editForm.editDate.value;
      const heure  = editForm.editHeure.value;
      const depart = editForm.editDepart.value.trim();
      const arrivee= editForm.editArrivee.value.trim();
      const places = parseInt(editForm.editPlaces.value, 10);
      const prix   = editForm.editPrix ? parseFloat(editForm.editPrix.value) : null;

      // Validation
      if (!id || !date || !heure || !depart || !arrivee || places <= 0 || prix === null || isNaN(prix) || prix < 0) {
        return alert('Veuillez remplir tous les champs du formulaire de modification.');
      }

      try {
        const response = await fetch('./asset/PHP/update_trajet.php', {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
