document.addEventListener('DOMContentLoaded', () => {
  ...
  // Charger et afficher la liste des conversations
  function chargerConversations() {
    fetch('asset/PHP/get_messages_recus.php')
      ...
      .then(data => {
        ...
        // Si pas encore de conversation sélectionnée, sélectionner la 1ère
  ...
  function chargerConversation(userId) {
    if (!userId) {
      messageThread.innerHTML = '<p class="text-muted">Sélectionnez un destinataire.</p>';
      return;
    }
    fetch('asset/PHP/get_conversation.php?with=' + userId)
      .then(res => res.json())
      .then(data => {
        messageThread.innerHTML = '';
  ...
  sendBtn.onclick = () => {
    ...
    if (!message || !destinataire) return alert('Veuillez sélectionner un destinataire et écrire un message.');
    ...
    fetch('asset/PHP/envoyer_message.php', {
      ...
      .then(data => {
        if (data.success) {
          ...
          chargerConversations(); // Pour rafraîchir badges et liste
        } else {
  ...
  // Bouton rafraîchir la conversation
  if (refreshBtn) {
    refreshBtn.onclick = () => {
