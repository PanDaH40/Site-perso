...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function afficherTousLesTrajets() {
    ...
    const trajetsDisponibles = trajets.filter(t => t.places > 0);
    ...
    trajetsDisponibles.forEach((t, index) => {
      ...
      const statut = t.total_reservations > 0 ?
        `<span class='badge bg-info'>Réservé (${t.total_reservations}/${t.places})</span>` :
  ...
  window.ouvrirReservation = function(index) {
    trajetSelectionne = trajets.filter(t => t.places > 0)[index];
