...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function afficherTousLesTrajets() {
    ...
    const trajetsDisponibles = trajets.filter(t => t.places > 0 && (t.places - (t.total_reservations || 0) > 0));

    if (trajetsDisponibles.length === 0) {
      trajetList.innerHTML = "<p class='text-muted'>Aucun trajet disponible.</p>";
      return;
    }

    trajetsDisponibles.forEach((t, index) => {
      const div = document.createElement("div");
      div.classList.add("col-12", "mb-3");
      const statut = t.total_reservations > 0
        ? `<span class='badge bg-info'>Réservé (${t.total_reservations}/${t.places})</span><br><strong>Réservé par :</strong> ${t.reservataires || '---'}`
      ...
      div.innerHTML = `
        <div class="card p-3">
          ...
          Places disponibles : ${t.places - (t.total_reservations || 0)}<br>
          Statut : ${statut}<br>
  ...
  window.ouvrirReservation = function(index) {
    trajetSelectionne = trajets.filter(t => t.places > 0 && (t.places - (t.total_reservations || 0) > 0))[index];
    const details = `${trajetSelectionne.date} ${trajetSelectionne.heure || ""}<br>${trajetSelectionne.depart} → ${trajetSelectionne.arrivee}<br>Places disponibles : ${trajetSelectionne.places - (trajetSelectionne.total_reservations || 0)}`;
  ...
  function chargerTousLesTrajets() {
    fetch("./asset/PHP/trajets.php?all=1", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        trajets = (data.all_trajets || []).filter(t => t.places - (t.total_reservations || 0) > 0);
