document.addEventListener("DOMContentLoaded", () => {
  ...
  window.adminResetPwd = function(id) {
    ...
    .then(res => res.json())
    .then(data => {
      if (data.success && data.nv_mdp) {
        feedback.textContent = "MDP réinitialisé : " + data.nv_mdp;
        feedback.className = "text-success";
      } else {
        feedback.textContent = data.error || "Erreur lors de la réinitialisation du MDP.";
        feedback.className = "text-danger";
      }
    })
    .catch(error => {
  ...
  window.adminEditJetons = function(id, jetonsActuels) {
    ...
    fetch('asset/PHP/admin_update_jetons.php', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
