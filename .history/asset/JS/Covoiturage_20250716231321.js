  ...
  fetch(url, { credentials: 'include' })
    ...
    .then(data => {
      if (Array.isArray(data)) {
        ...
        if (data.all_trajets.length === 0 && data.date_alternative) {
          if (confirm(`Aucun trajet trouvé à cette date. Voulez-vous voir les trajets pour la date alternative : ${data.date_alternative} ?`)) {
            document.getElementById("dateSearch").value = formatDateForInput(data.date_alternative);
            rechercherTrajets();
          } else {
            afficherAucunTrajet();
          }
...
// Fonction utilitaire pour convertir une date format "d/m/Y" vers "YYYY-MM-DD" (format input[type=date])
