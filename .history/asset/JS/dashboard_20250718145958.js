...
document.addEventListener('DOMContentLoaded', () => {
  // Charge toutes les données du dashboard au démarrage
  chargerDashboard();

  // Met à jour le badge de notification des messages non lus
  mettreAJourNotificationMessages();

  // Charge aussi la liste des trajets à valider (passager)
  ...
  // Actualisation automatique toutes les 30 sec du badge notification messages
  setInterval(mettreAJourNotificationMessages, 30000);
});

// Met à jour la notification messages (badge)
...
// Charge les données du dashboard (trajets proposés, réservés, demandes)
...
// Affiche statistiques basiques (nombre de trajets proposés, réservés, places)
...
// Affiche la liste des trajets proposés (avec boutons d'état et actions)
function afficherTrajetsProposes(trajets) {
  ...
  trajets.forEach(trajet => {
    ...
    // Boutons d'état selon l'état actuel du trajet
    let btnEtatHTML = '';
  ...
  // Ajout des listeners sur boutons créés
...
// Affiche trajets réservés (avec possibilité d’annuler)
function afficherTrajetsReserves(trajets) {
  ...
  // Listeners pour annuler les réservations
...
function afficherDemandesReservations(demandes) {
  ...
  // Listeners accepter/refuser demandes
...
// Affichage de l'avatar et nom du conducteur pour un trajet
...
// Formatage date et heure en affichage lisible français
...
// Ouvre le modal de modification et pré-remplit les champs avec les données du trajet
...
// Supprimer un trajet (confirmation et appel API)
function supprimerTrajet(trajetId) {
  ...
  .then(data => {
    if (data.success) {
      ...
      chargerDashboard();  // recharge le dashboard pour mettre à jour l’affichage
      chargerTrajets();    // recharge la liste des trajets visibles
    } else {
...
// Annuler une réservation (confirmation et appel API)
...
// Changer l'état d'un trajet (démarrer, terminer)
function changerEtatTrajet(trajetId, nouvelEtat) {
  ...
  .then(data => {
    if (data.success) {
      ...
      chargerDashboard();
      chargerTrajetsAValider(); // actualise aussi la liste des validations passager
    } else {
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (result.success) {
            ...
            chargerTrajetsAValider();
            chargerDashboard();
            if (window.actualiserCredits) {
              window.actualiserCredits();  // <-- Ici on rafraîchit les crédits profil et notif
            }
          } else {
