document.addEventListener("DOMContentLoaded", () => {
  ...
  function chargerMembres() {
    ...
    fetchJson(`asset/PHP/admin_get_users.php?search=${encodeURIComponent(searchInput.value)}&role=${roleFilter.value}&statut=${statutFilter.value}`)
      .then(data => {
        if (data.error) return tbodyUsers.innerHTML = `<tr><td colspan='9' class='text-danger text-center'>${data.error}</td></tr>`;
        if (!Array.isArray(data.users) || data.users.length === 0) 
          return tbodyUsers.innerHTML = `<tr><td colspan='9' class='text-center'>Aucun membre trouvé.</td></tr>`;
  ...
  function chargerSignalements() {
    ...
    fetchJson('asset/PHP/get_signalements.php')
      .then(data => {
        if (data.error) return tbodySignalements.innerHTML = `<tr><td colspan='8' class='text-danger text-center'>${data.error}</td></tr>`;
        if (!Array.isArray(data) || data.length === 0) 
          return tbodySignalements.innerHTML = `<tr><td colspan='8' class='text-center'>Aucun signalement trouvé.</td></tr>`;

        tbodySignalements.innerHTML = "";
        data.forEach(s => {
          const statutLabel = s.statut === 'en_attente' 
          ...
          tbodySignalements.insertAdjacentHTML('beforeend', `
            <tr>
              ...
              <td>
                ...
                ${s.statut === 'en_attente' ? `<button class="btn btn-success btn-sm ms-1" onclick="traiterSignalement(${s.id})" title="Marquer comme traité">
  ...
  window.traiterSignalement = id => {
    ...
    .then(data => {
      if (data.success) {
        feedback.textContent = "Signalement marqué comme traité.";
        feedback.className = "text-success";
        chargerSignalements();
      } else {
        feedback.textContent = data.error || "Erreur lors de la mise à jour du signalement.";
        feedback.className = "text-danger";
      }
    })
    .catch(() => {
      feedback.textContent = "Erreur réseau lors de la mise à jour du signalement.";
      feedback.className = "text-danger";
    });
  };
