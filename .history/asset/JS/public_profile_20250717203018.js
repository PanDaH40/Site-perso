document.addEventListener('DOMContentLoaded', () => { 
  ...
  // Fonction pour afficher l'avatar avec fallback image par défaut
  ...
  // Fonction pour afficher les étoiles (note moyenne)
  function renderStars(moyenne, nbAvis = 0) {
    ...
    for (let i = 1; i <= 5; i++) {
      if (note >= i) {
        container.innerHTML += '<span style="color:#ffc107;font-size:1.3rem;">★</span>';  // pleine
      } else if (note >= i - 0.5) {
        container.innerHTML += '<span style="color:#ffc107;font-size:1.3rem;">☆</span>';  // demi étoile (ici simple étoile vide pour simplifier)
      } else {
        container.innerHTML += '<span style="color:#bbb;font-size:1.3rem;">☆</span>';  // vide
      }
    }
    if (nbAvis > 0) {
  ...
  // Fonction qui crée le formulaire pour donner ou modifier un avis
  ...
  fetch(`asset/PHP/get_public_profile.php?id=${encodeURIComponent(userId)}`)
    ...
    .then(data => {
      ...
      if (data.roleConducteur) {
        ...
        // === Ajout du bouton "Historique des trajets" (juste ici) ===
        const historiqueBtn = document.createElement("a");
        historiqueBtn.href = `HistoriqueConducteur.html?id=${userId}`;
        historiqueBtn.className = "btn btn-outline-primary mt-3";
        historiqueBtn.textContent = "Historique des trajets";
        infosPlusEl.appendChild(historiqueBtn);
      }
