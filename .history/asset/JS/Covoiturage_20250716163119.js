document.addEventListener("DOMContentLoaded", () => {
  // --- 0. Fonction pour mettre à jour la notification globale des messages ---
  function mettreAJourNotificationMessages() {
    const notifBadge = document.getElementById('notif-badge'); // Assure-toi que l'ID correspond à ta cloche dans HTML
    if (!notifBadge) return; // Pas de badge => pas besoin
  ...
  fetch('asset/PHP/get_profile.php', { credentials: 'include' })
    ...
    .catch(() => {
      // En cas d’erreur, on masque le formulaire pour éviter bugs
      const formAjout = document.getElementById("formAjoutContainer");
      if (formAjout) formAjout.style.display = "none";
    });

  // --- 2. Charger et afficher les trajets ---
  chargerTrajets();
  ...
  // --- 4. Gestion du formulaire d'ajout de trajet ---
  ...
  // --- 5. Mettre à jour la notification messages au chargement ---
