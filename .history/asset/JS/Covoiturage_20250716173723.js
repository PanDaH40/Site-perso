...
function afficherTrajets(trajets) {
  ...
  trajets.forEach(trajet => {
    ...
    // Ici on récupère le type de carburant du trajet, par exemple "electric", "essence", "gazole"
    const typeCarburant = trajet.typeCarburant || ""; // Assure-toi que côté PHP tu envoies bien cette clé

    // Critère écologique : covoiturage (2+ passagers) OU véhicule électrique/hybride
    const estEcologique = (trajet.nb_passagers >= 2) ||
      (typeCarburant && ['electric', 'électrique', 'hybride'].includes(typeCarburant.toLowerCase()));

    // Badge vert écologique si critère rempli
  ...
  // Ajout des événements clic sur boutons Réserver
