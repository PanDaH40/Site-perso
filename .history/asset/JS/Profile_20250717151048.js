document.addEventListener('DOMContentLoaded', () => {
  ...
  // Badge notification (attention au nom id)
  const notifBadge       = $('badge-msg') || $('notif-badge');

  // Nouveaux champs pour véhicule conducteur :
  const marqueVehiculeInput = $('profileMarqueVehicule');
  const modeleVehiculeInput = $('profileModeleVehicule');
  const plaqueInput   = document.getElementById('profilePlaque');
  const couleurInput  = document.getElementById('profileCouleur');
  const dateImmatInput = document.getElementById('profileDateImmat');
  ...
  function toggleRoleSections() {
    if (roleCondCheckbox && conducteurFields) {
      ...
      // Champs véhicule conducteur (modèle et marque)
      ...
      if (voitureInput) voitureInput.required = !!roleCondCheckbox.checked;
  ...
  async function loadProfile() {
    try {
      ...
      // Champs véhicule (marque et modèle)
      ...
      if (data.credits !== undefined && $('profileCredits')) {
        $('profileCredits').textContent = data.credits;
      }
  ...
  if (form) {
    form.addEventListener('submit', async e => {
      ...
      if (roleCondCheckbox && roleCondCheckbox.checked) {
        ...
        if (plaqueInput)      fd.append('plaque', plaqueInput.value.trim());
        if (couleurInput)     fd.append('couleur', couleurInput.value.trim());
        if (dateImmatInput)   fd.append('date_premiere_immatriculation', dateImmatInput.value);
      }
