document.addEventListener("DOMContentLoaded", () => {
  ...
  function mettreAJourNotificationMessages() {
    ...
    fetch(basePath + 'get_messages_recus.php', { credentials: 'include' })
      .then(res => res.ok ? res.json() : {totalNonLus: 0})
      .then(data => {
  ...
  // --- Affichage profil/messages selon session ---
  ...
  // --- Vérifie si utilisateur est conducteur avant d'afficher le formulaire ajout trajet ---
  ...
  // --- Formulaire ajout trajet (conducteur) ---
  ...
  // --- Modal réservation avec double confirmation ---
  function ouvrirModalReservation(trajetId) {
    ...
    btnConfirmerNew.onclick = function () {
      ...
      fetch(basePath + 'reserver.php', {
        ...
        credentials: 'include',
        body: JSON.stringify({ trajet_id: trajetId, places: places })
      })
        .then(r => r.json())
        .then(data => {
          modal.hide();
