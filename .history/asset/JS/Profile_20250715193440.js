...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Affiche/masque les sections conducteur/passager selon checkbox
  function toggleRoleSections() {
    ...
    if (preferencesInput) preferencesInput.required = false; // toujours facultatif
  }
  ...
  // Cliquer sur l’avatar ouvre le file-picker
  ...
  // Charger et pré-remplir le formulaire profil
  async function loadProfile() {
    try {
      ...
      if (roleCondCheckbox) roleCondCheckbox.checked = Boolean(data.cond_prenom);
      if (rolePassCheckbox) rolePassCheckbox.checked = Boolean(data.pass_prenom);
      toggleRoleSections();
      ...
      if (animauxCheckbox)   animauxCheckbox.checked = Boolean(data.animaux);
      if (fumeurCheckbox)    fumeurCheckbox.checked  = Boolean(data.fumeurs);

      if (preferencesInput) preferencesInput.value = data.preferences || '';

      if (avatarPreview) {
        avatarPreview.src = data.avatar && data.avatar !== 'null' ? data.avatar : defaultAvatar;
  ...
  // Soumission du formulaire avec avatar (FormData)
