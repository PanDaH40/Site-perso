document.addEventListener("DOMContentLoaded", () => {
  ...
  function chargerSignalements() {
    if (!tbodySignalements) return; // Sécurité si table absente
    tbodySignalements.innerHTML = `<tr><td colspan='8' class='text-center'>Chargement...</td></tr>`;

    fetchJson('asset/PHP/get_signalement.php')
      .then(data => {
        ...
        data.forEach(s => {
          tbodySignalements.insertAdjacentHTML('beforeend', `
            <tr>
              ...
              <td>${s.statut === 0 ? '<span class="badge bg-warning">Non traité</span>' : '<span class="badge bg-success">Traité</span>'}</td>
              <td>
