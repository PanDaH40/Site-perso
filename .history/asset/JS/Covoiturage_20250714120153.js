document.addEventListener("DOMContentLoaded", () => {
  ...
  const filtreDepart = searchParams.get("departure")?.toLowerCase();
  const filtreArrivee = searchParams.get("arrival")?.toLowerCase();
  const filtreDate = searchParams.get("date");
  const filtrePlaces = parseInt(searchParams.get("passengers"));
    ...
    fetch("./asset/PHP/trajets.php", { credentials: "same-origin" })
      ...
      .then(data => {
        ...
        if (proposesBody && Array.isArray(data.trajets_proposes)) {
          const filtres = data.trajets_proposes.filter(t => {
            return (
              (!filtreDepart || t.depart.toLowerCase().includes(filtreDepart)) &&
              (!filtreArrivee || t.arrivee.toLowerCase().includes(filtreArrivee)) &&
              (!filtreDate || t.date === filtreDate) &&
              (!filtrePlaces || (t.places - (parseInt(t.total_reservations) || 0)) >= filtrePlaces)
            );
          });
        ...
        if (reservesBody && Array.isArray(data.trajets_reserves)) {
          if (data.trajets_reserves.length === 0) {
            ...
            data.trajets_reserves.forEach(t => {
              const nbPlaces = t.places_reservees ?? '?';
              const statut = `${t.statut} - ${nbPlaces} place(s) réservée(s)`;
              const row = document.createElement("tr");
              row.innerHTML = `
                <td>${t.date} ${t.heure}</td>
                <td>${t.depart}</td>
