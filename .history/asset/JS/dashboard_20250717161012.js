...
function afficherTrajetsProposes(trajets) {
  ...
  trajets.forEach(trajet => {
    ...
    // Texte de l'état à afficher + bouton
    let etatTexte = '';
    let boutonEtat = '';

    switch(trajet.etat_trajet) {
      case 'planifie':
        etatTexte = 'Planifié';
        boutonEtat = `<button class="btn btn-sm btn-success btn-start" data-id="${trajet.id}">Démarrer</button>`;
        break;
      case 'en_cours':
        etatTexte = 'En cours';
        boutonEtat = `<button class="btn btn-sm btn-primary btn-end" data-id="${trajet.id}">Arrivée à destination</button>`;
        break;
      case 'termine':
        etatTexte = 'Terminé';
        boutonEtat = '';
  ...
  // Event listeners pour boutons démarrer / arrivée
...
async function changerEtatTrajet(id, nouvelEtat) {
  try {
    const res = await fetch('asset/PHP/update_trajet_etat.php', {
      method: 'POST',
      credentials: 'include',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ id, etat_trajet: nouvelEtat })
    });
    const data = await res.json();
    if (data.success) {
      alert('État du trajet mis à jour.');
      chargerDashboard();
    } else {
