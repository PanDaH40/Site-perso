...
try {
    ...
    // Récupération de tous les trajets publics avec total_reservations
    if (isset($_GET['all']) && $_GET['all'] == '1') {
        $stmt = $pdo->query("
            SELECT 
                ...
                COALESCE(SUM(r.places), 0) AS total_reservations
            FROM trajets t
    ...
    if ($userId) {
        ...
        $stmt = $pdo->prepare("
            SELECT 
                t.*, 
                (SELECT COALESCE(SUM(r.places), 0) FROM reservations r WHERE r.trajet_id = t.id) AS total_reservations
            FROM trajets t
