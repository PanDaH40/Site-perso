...
try {
    ...
    if (isset($_POST['prenom']) && isset($_POST['nom']) && isset($_POST['email'])) {
        ...
        if (!empty($_POST['roleConducteur'])) {
            // On insère ou update la fiche conducteur AVEC marque et modèle
            $stmt = $pdo->prepare("
  INSERT INTO conducteurs (
    inscrit_id, marque_vehicule, modele_vehicule, carburant, animaux, fumeurs, 
    plaque, couleur, date_premiere_immatriculation
  )
  VALUES (:id, :marque, :modele, :carburant, :animaux, :fumeurs, :plaque, :couleur, :dateimmat)
  ON DUPLICATE KEY UPDATE
    marque_vehicule=:marque, modele_vehicule=:modele, carburant=:carburant, animaux=:animaux, fumeurs=:fumeurs,
    plaque=:plaque, couleur=:couleur, date_premiere_immatriculation=:dateimmat
");
$stmt->execute([
  ':id'        => $userId,
