...
try {
    ...
    // Tous les trajets publics avec total réservations
    if (isset($_GET['all']) && $_GET['all'] == '1') {
        $stmt = $pdo->query("SELECT t.*, u.prenom AS conducteur_prenom, u.nom AS conducteur_nom,
    ...
    if ($userId) {
        // Trajets proposés
        $stmt = $pdo->prepare("SELECT t.*, COALESCE(SUM(r.places_reservees), 0) AS total_reservations
                               FROM trajets t
                               LEFT JOIN reservations r ON r.trajet_id = t.id
        ...
        foreach ($trajetsProposes as &$trajet) {
            $stmt = $pdo->prepare("SELECT i.prenom, i.nom, r.places_reservees
                                   FROM reservations r
