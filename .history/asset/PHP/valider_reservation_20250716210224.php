...
// Supporter aussi "id" en plus de "reservation_id" côté JS au cas où
$reservationId = 0;
...
try {
    ...
    if ($action === 'accepter') {
        ...
        $success = $stmtUpdate->execute([$reservationId]);

        if (!$success || $stmtUpdate->rowCount() === 0) {
            $pdo->rollBack();
            http_response_code(500);
            echo json_encode(['error' => 'Impossible de mettre à jour la réservation']);
            exit;
        }
        $pdo->commit();
