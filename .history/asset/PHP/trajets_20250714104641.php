...
try {
    ...
    // Tous les trajets publics avec le total de places réservées
    ...
    // Trajets proposés et réservés par l'utilisateur connecté
    $trajetsProposes = [];
    $trajetsReserves = [];

    if ($userId) {
        // Trajets proposés + réservations
        $stmt = $pdo->prepare("
            SELECT 
                t.*, 
                COALESCE(SUM(r.places_reservees), 0) AS total_reservations
        ...
        // Ajouter les détails de qui a réservé quoi
