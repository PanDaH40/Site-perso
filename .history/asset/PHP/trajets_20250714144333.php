...
try {
    ...
    if ($userId) {
        // a) Trajets proposés
        $stmt = $pdo->prepare("SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.places, COALESCE(SUM(r.places_reservees), 0) AS total_reservations
                               FROM trajets t
                               LEFT JOIN reservations r ON r.trajet_id = t.id
        ...
        // Pour chaque trajet proposé, récupérer nom + places réservées
        foreach ($trajetsProposes as &$trajet) {
            ...
            $trajet['statut_conducteur'] = !empty($list)
                ? implode(', ', $list)
                : 'Aucune réservation';
        }

        // b) Trajets réservés
        $stmt = $pdo->prepare("SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.statut,
