...
try {
    ...
    // Vérifier réservation existante/passager
    ...
    if ($action === 'valider') {
        // Mettre à jour validation_passager, note et avis
        $pdo->prepare("UPDATE reservations SET validation_passager=1, note=?, avis=?, avis_valide=0, date_validation=NOW() WHERE id=?")
            ->execute([$note ?: null, $avis, $reservationId]);
        ...
        $pdo->prepare("INSERT INTO signalements (signale_par, cible, type, motif, description, statut, date_signalement) 
            ...
            ->execute([$userId, $reservationId, $commentaire]);
    ...
    if ($action === 'valider') {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM reservations WHERE trajet_id=? AND statut='valide' AND (validation_passager IS NULL OR validation_passager!=1)");
