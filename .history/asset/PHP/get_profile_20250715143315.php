...
// 3) Récupération du profil avec conducteurs/passagers
try {
    $sql = "
      SELECT 
    ...
    $profile = [
        ...
        'nom'         => $row['user_nom'],
        'email'       => $row['email'],
        // Conducteur (peut être null)
        'cond_prenom' => $row['cond_prenom']   ?? '',
        'cond_nom'    => $row['cond_nom']      ?? '',
        'voiture'     => $row['voiture']       ?? '',
        'carburant'   => $row['carburant']     ?? '',
        'animaux'     => isset($row['animaux']) ? (int)$row['animaux'] : 0,
        'fumeurs'     => isset($row['fumeurs']) ? (int)$row['fumeurs'] : 0,
        // Passager (peut être null)
