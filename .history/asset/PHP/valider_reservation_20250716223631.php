...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT r.trajet_id, r.places_reservees, r.statut, t.conducteur_id, t.places,
            (SELECT COALESCE(SUM(CASE WHEN statut = 'Confirmé' THEN places_reservees ELSE 0 END),0) FROM reservations WHERE trajet_id = t.id) AS places_deja_reservees,
            r.passager_id
        FROM reservations r
    ...
    if ($action === 'accepter') {
        ...
        $credits_a_deduire = 2; // 2 crédits par réservation (tu peux ajuster selon prix etc.)
        ...
        $stmtUpdate = $pdo->prepare("UPDATE reservations SET statut = 'Confirmé' WHERE id = ?");
        ...
        $places_nouvelles = $res['places'] - $res['places_reservees'];
        $stmtUpdateTrajet = $pdo->prepare("UPDATE trajets SET places = ? WHERE id = ?");
        $stmtUpdateTrajet->execute([$places_nouvelles, $res['trajet_id']]);

        $pdo->commit();
        echo json_encode(['success' => true, 'message' => 'Réservation acceptée']);
    } else { // refuser
        $stmtUpdate = $pdo->prepare("UPDATE reservations SET statut = 'Annulé' WHERE id = ?");
        $stmtUpdate->execute([$reservationId]);
