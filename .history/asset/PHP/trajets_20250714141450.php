...
try {
    ...
    if ($userId) {
        // Trajets proposés
        $stmt = $pdo->prepare("SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.places,
                                       COALESCE(SUM(r.places_reservees), 0) AS total_reservations
                                FROM trajets t
                                LEFT JOIN reservations r ON r.trajet_id = t.id
        ...
        // Récupérer les réservataires
        foreach ($trajetsProposes as &$trajet) {
            $qr = $pdo->prepare("SELECT i.prenom, i.nom FROM reservations r JOIN inscrits i ON i.id = r.passager_id WHERE r.trajet_id = ?");
