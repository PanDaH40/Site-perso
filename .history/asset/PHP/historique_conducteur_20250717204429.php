...
if (!$conducteurId || !is_numeric($conducteurId)) {
    echo json_encode(['error' => 'ID conducteur manquant ou invalide']);
    exit;
}

// Liste des trajets terminés (date passée)
try {
    $sql = "SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.places, t.jetons,
            ...
            FROM trajets t
            WHERE t.conducteur_id = ? AND CONCAT(t.date, ' ', t.heure) < NOW()
