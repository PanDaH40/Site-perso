...
";
        ...
        if (empty($all) && !empty($_GET['date'])) {
            $stmtAlt = $pdo->prepare("
                ...
                WHERE CONCAT(t.date, ' ', t.heure) > NOW()
                AND (t.places - (SELECT COALESCE(SUM(r.places_reservees),0) FROM reservations r WHERE r.trajet_id = t.id AND r.statut = 'valide')) > 0
                ORDER BY t.date ASC, t.heure ASC
                LIMIT 1
            ");
            $stmtAlt->execute();
            ...
            echo json_encode([
                ...
                'message' => $altDate ? "Aucun trajet disponible à cette date. Essayez la date du " . date('d/m/Y', strtotime($altDate)) : "Aucun trajet disponible."
            ]);
            exit;
        }
