...
// 2) Liste publique de tous les trajets filtrés (places > 0 et à venir)
if (isset($_GET['all']) && $_GET['all'] === '1') {
    try {
        // Préparation dynamique des filtres
        $where = [];
        $params = [];

        // Obligatoire : places restantes > 0 et trajet à venir
        $where[] = "(t.places - (SELECT COALESCE(SUM(r.places_reservees),0) FROM reservations r WHERE r.trajet_id = t.id)) > 0";
        $where[] = "CONCAT(t.date, ' ', t.heure) > NOW()";

        // Filtres facultatifs (en fonction du formulaire)
        ...
        if (!empty($_GET['places'])) {
            $where[] = "(t.places - (SELECT COALESCE(SUM(r.places_reservees),0) FROM reservations r WHERE r.trajet_id = t.id)) >= ?";
            $params[] = intval($_GET['places']);
        }
...
try {
    // a) Trajets proposés par le conducteur (futurs uniquement)
    $stmt1 = $pdo->prepare(
        "SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.places, t.prix,
        (SELECT COALESCE(SUM(r.places_reservees),0) FROM reservations r WHERE r.trajet_id = t.id) AS total_reservations
        FROM trajets t
    ...
    // b) Trajets réservés par le passager (futurs uniquement)
    $stmt2 = $pdo->prepare(
        "SELECT t.id, t.date, t.heure, t.depart, t.arrivee, t.places, t.prix,
         r.places_reservees,
         c.prenom AS conducteur_prenom, c.nom AS conducteur_nom, i.avatar AS conducteur_avatar
