...
try {
    ...
    // === DASHBOARD : PROPOSÉS ET RÉSERVÉS ===
    if ($userId) {
        // Trajets proposés
        $stmt = $pdo->prepare(
        ...
        // Ajouter réservataires et nombre places réservées aux trajets proposés
        foreach ($trajetsProposes as &$trajet) {
            ...
            // Préparer une liste de "Prénom Nom (X places)"
            $list = [];
            foreach ($reservations as $r) {
                $list[] = "{$r['prenom']} {$r['nom']} ({$r['places_reservees']})";
            }
            $trajet['reservataires'] = $list;
        }
