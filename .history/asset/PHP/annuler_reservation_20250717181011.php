...
if (isset($input['id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare('SELECT statut, trajet_id, passager_id, places_reservees FROM reservations WHERE id = ? AND passager_id = ?');
        ...
        if ($reservation['statut'] === 'valide') {
            ...
            if ($trajet) {
                ...
                // Retire les jetons du conducteur (prix du trajet * places)
                $stmt = $pdo->prepare('UPDATE inscrits SET credits = credits - ? WHERE id = ?');
                $stmt->execute([$jetons_conducteur, $trajet['conducteur_id']]);
