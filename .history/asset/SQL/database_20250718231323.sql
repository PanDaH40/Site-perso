...
ALTER TABLE conducteurs ADD COLUMN marque_vehicule VARCHAR(50) DEFAULT NULL;

ALTER TABLE conducteurs CHANGE voiture modele_vehicule VARCHAR(80);

SHOW CREATE TABLE conducteurs;

ALTER TABLE conducteurs
  ADD COLUMN plaque VARCHAR(20) DEFAULT NULL,
  ADD COLUMN couleur VARCHAR(30) DEFAULT NULL,
  ADD COLUMN date_premiere_immatriculation DATE DEFAULT NULL;

ALTER TABLE trajets ADD etat_trajet ENUM('planifie','en_cours','termine') NOT NULL DEFAULT 'planifie';

ALTER TABLE reservations
ADD validation_passager TINYINT(1) NULL DEFAULT 0 AFTER statut;

SELECT * FROM trajets WHERE conducteur_id = 1 ORDER BY date DESC, heure DESC;

ALTER TABLE inscrits
ADD COLUMN admin TINYINT(1) NOT NULL DEFAULT 0;

-- Ajouter admin et statut
ALTER TABLE inscrits ADD COLUMN admin TINYINT(1) NOT NULL DEFAULT 0;
ALTER TABLE inscrits ADD COLUMN statut ENUM('actif','suspendu') NOT NULL DEFAULT 'actif';
...
ALTER TABLE `reservations`
  ADD COLUMN `note` TINYINT DEFAULT NULL,
  ADD COLUMN `avis` TEXT DEFAULT NULL,
  ADD COLUMN `avis_valide` TINYINT DEFAULT 0, -- 0: en attente, 1: ok admin
  ADD COLUMN `date_validation` DATETIME DEFAULT NULL;

ALTER TABLE signalements ADD COLUMN statut VARCHAR(20) DEFAULT 'ouvert';

ALTER TABLE signalements
ADD COLUMN statut ENUM('en_attente', 'termine') NOT NULL DEFAULT 'en_attente';

ALTER TABLE signalements
MODIFY statut ENUM('en_attente', 'traite') NOT NULL DEFAULT 'en_attente';
